...
#ifndef RACECARPLANNER_GLOBAL_PLANNER_TRACK_HPP
#define RACECARPLANNER_GLOBAL_PLANNER_TRACK_HPP
...
namespace acsr{
    ...
    class BicycleDynamicsTwoBrakeGlobalPlanner{
    public:
        BicycleDynamicsTwoBrakeGlobalPlanner() = default;
        BicycleDynamicsTwoBrakeGlobalPlanner(const std::string& track_file,const std::string& vehicle_file,const std::string& front_tire_file,const std::string& rear_tire_file){
        ...
        void plan_refine(double start_s = 0, double end_s = -1,double n0 =0, double v0=0.15,int N = 100,bool print = true, bool save_to_database = true){
            auto all = Slice();
            auto _0_N = Slice(0,N);
            ...
            const auto track_width = track->get_width();
            ...
            const auto tau_array = track->s_to_t_lookup(s_val)[0];
            const auto tau0 = double(tau_array(0));
            const auto phi0 = double(track->f_phi(tau_array(0))[0]);
            const auto tau_array_mid = track->s_to_t_lookup((s_val(0,_0_N)+s_val(0,_N_1))*0.5);

            const auto kappa_array = track->f_kappa(tau_array_mid)[0];
            //const auto kappa_array = (kappa_array_temp(0,_N_1) + kappa_array_temp(0,Slice(1,N+1)))*0.5;
            const auto tangent_vec_array= track->f_tangent_vec(tau_array_mid)[0];
        ...
        void plan(double start_s = 0, double end_s = -1,double n0 =0, double v0=0.15,int N = 100,bool print = true, bool save_to_database = true){
            auto all = Slice();
            auto _N_1 = Slice(0,N);
            ...
            const auto s_val = casadi::DM::linspace(start_s,end_s,N+1).T();
            const auto tau_array = track->s_to_t_lookup(s_val)[0];
            const auto kappa_array = track->f_kappa(tau_array)[0];
            const auto tangent_vec_array= track->f_tangent_vec(tau_array)[0];
            const auto phi_array = DM::atan2(tangent_vec_array(1,all),tangent_vec_array(0,all));
            const auto tangent_vec_norm = DM::sqrt((tangent_vec_array(0,all)*tangent_vec_array(0,all)+tangent_vec_array(1,all)*tangent_vec_array(1,all)));
    ...
    private:
        ...
        void save(DM& dt_array,DM& x,DM& u){
...

            {
                ...
                auto N = x.columns()-1;
                auto dm_xy = track->f_tn_to_xy(std::vector<DM>{x(0,Slice()),x(1,Slice())})[0];
                auto s_array = track->t_to_s_lookup(x(0,Slice()))[0];
    ...
    class BicycleDynamicsOneBrakeGlobalPlanner{
    public:
        BicycleDynamicsOneBrakeGlobalPlanner() = default;
        BicycleDynamicsOneBrakeGlobalPlanner(const std::string& track_file,const std::string& vehicle_file,const std::string& front_tire_file,const std::string& rear_tire_file){
            if(!std::filesystem::exists(vehicle_file)){
                std::cout<<vehicle_file<<" does not exist\n";
                return;
            }
        ...
        void plan(double start_s = 0, double end_s = -1,double n0 =0, double v0=0.15,int N = 100,bool print = true, bool save_to_database = true){
            ...
            auto throttle_sym_array = U(IDX_U_Throttle,all);
            auto brake_sym_array = U(IDX_U_Brake,all);
            //auto rear_brake_sym_array = U(IDX_U_Brake,all);
            ...
            X_dot(1, all) = dt_sym_array*(vx_sym_array * MX::sin(dphi_c_sym_array) + vy_sym_array* MX::cos(dphi_c_sym_array)) ;
            X_dot(2, all) = dt_sym_array*omega_sym_array;
            X_dot(3, all) = dt_sym_array * (fx_sym_array + fx_sym_array * MX::cos(delta_sym_array) - fy_f_sym_array * MX::sin(delta_sym_array) + mass * vy_sym_array* omega_sym_array)/ mass;
            X_dot(4, all) = dt_sym_array * (fy_r_sym_array + fx_sym_array * MX::sin(delta_sym_array) + fy_f_sym_array * MX::cos(delta_sym_array) - mass * vx_sym_array * omega_sym_array)/ mass;
            X_dot(5, all) = dt_sym_array * (fy_f_sym_array * lf * MX::cos(delta_sym_array) + fx_sym_array * lf * MX::sin(delta_sym_array) - fy_r_sym_array * lr)/ Iz ;
            X_dot(6, all) = dt_sym_array * delta_dot_sym_array;
        ...
        void plot_trajectory(const std::shared_ptr<Track>& plot_track = nullptr,const std::string& table_name=std::string()){
            ...
            std::vector<double> delta_vec;
            std::vector<double> throttle_vec;
            std::vector<double> brake_vec;
            //std::vector<double> bf_vec;

            SQLite::Statement query(db, "SELECT dt,tau,s,n,x,y,phi,vx,vy,delta,throttle,brake FROM "+table_);
            while (query.executeStep())
            {
                dt_vec.push_back(query.getColumn(0));
                tau_vec.push_back(query.getColumn(1));
                ...
                delta_vec.push_back(query.getColumn(9));
                throttle_vec.push_back(query.getColumn(10));
                brake_vec.push_back(query.getColumn(11));
            }
            ...
            plt::figure(3);
            plt::named_plot("throttle",t,throttle_vec,"g-.");
            plt::named_plot("brake",t,brake_vec,"k--");
            //plt::named_plot("rear brake",t,br_vec,"b-");
            plt::named_plot("steering",t,delta_vec,"r-*");
            plt::legend();
...
#endif //RACECARPLANNER_GLOBAL_PLANNER_TRACK_HPP
